...
class getAllCampaign(Resource):
    ...
    def get(self):
        try:
            ...
            if key_word == "ALL":
                campaign_list = query.limit(
                    limit_number_records).offset(offset).all()
                total_records = query.count()
            else:
                campaign_filtered = query.filter(
                    Campaigns.name.like(f"%{key_word}%"))
                campaign_list = query.filter(Campaigns.name.like(f"%{key_word}%")).limit(
                    limit_number_records).offset(offset).all()
    ...
    def delete(self, camp_id):
        ...
        try:
            ...
            camp = Campaigns.query.filter(
                Campaigns.campaign_id == camp_id).first()
            creative = Creatives.query.filter(
                Creatives.campaign_id == camp_id).first()
        ...
        try:
            ...
            if campaigns:
                for campaign in campaigns:
                    creative = Creatives.query.filter(
                        Creatives.campaign_id == campaigns.campaign_id).first()
                    tuple_campaign = [
                        {
                            "name": campaign.name,
                            "user_status": campaign.user_status,
